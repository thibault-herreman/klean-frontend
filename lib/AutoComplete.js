...
function AutoComplete(props) {
  ...
  const renderItem = ({item, index}) => {
    if (index === 0) {
      return (
        <Pressable style={styles.itemTop} onPress={() => {props.onPress(item.properties.label); props.setShowAutoComplete(false)}}>
          <Text style={styles.text}>{item.properties.label}</Text>
        </Pressable>
      )
    } else {
      return (
        <Pressable style={styles.item} onPress={() => {props.onPress(item.properties.label); props.setShowAutoComplete(false)}}>
          <Text style={styles.text}>{item.properties.label}</Text>
        </Pressable>
      )
    }
